...
Keep `kubernetes_version` here in sync with `talos_cluster.kubernetes_version` so nodes added later (scale-up) bootstrap at the correct version. See [`talos_cluster`](../resources/cluster.md) for the recommended upgrade workflow.
